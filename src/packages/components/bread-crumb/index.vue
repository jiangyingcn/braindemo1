<template>
  ...
  <el-breadcrumb separator="/">
    <el-breadcrumb-item><i class="el-icon-location-outline"><span>当前位置：</span></i>智脑后台管理系统</el-breadcrumb-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //          this.firstMenuLabel = o.submenu.label;
      //        }
      //      })
      //   }
      // },
      getBreadcrumb: function () {
        let matched = this.$route.matched[0].path;
        // console.log(this.$route)


        for (let i = 0; i < this.menuList.length; i++) {
          let o = this.menuList[i].menuGroup;
          o.forEach(v => {
            if (v.to === matched) {
              this.menuLabel = v.label;
            }
